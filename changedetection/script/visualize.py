...
def create_color_map(pred, gt):
    ...
    color_map[(pred == 1) & (gt == 1)] = [255, 255, 255]  # TP - 白色
    color_map[(pred == 0) & (gt == 0)] = [0, 0, 0]        # TN - 黑色
    color_map[(pred == 1) & (gt == 0)] = [255, 0, 0]      # FP - 红色
    color_map[(pred == 0) & (gt == 1)] = [0, 255, 0]      # FN - 绿色

    return color_map


def main(pth_path, test_dir, save_dir, decoder_depths=4, drop_rate=0.0):
    ...
    parser.add_argument('--decoder_depths', type=int, default=decoder_depths)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Visualize and evaluate change detection results")
    ...
    parser.add_argument('--decoder_depths', type=int, default=4, help='Decoder depth (default: 4)')
    parser.add_argument('--drop_rate', type=float, default=0.0, help='Dropout rate (default: 0.0)')
    args = parser.parse_args()

    model_name = os.path.basename(os.path.dirname(args.resume))
    save_dir = os.path.join(args.result_saved_path, model_name)

    main(args.resume, args.test_dataset_path, save_dir,
         decoder_depths=args.decoder_depths, drop_rate=args.drop_rate)
