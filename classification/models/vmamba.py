...
try:
    ...
    try:
        from classification.models.csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1
    except ImportError:
...
class SelectiveScanMamba(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, backnrows=1, oflex=True):
        ...
        if selective_scan_cuda is None:
            # Fallback to PyTorch pure implementation
            out = selective_scan_ref_v2(u, delta, A, B, C, D, None, delta_bias, delta_softplus)
            # Not saving for backward because autograd will handle it for pure PyTorch if it's not a custom backward?
            # Wait, since it's inside a custom Function, we actually should compute it normally without custom backward,
            # or we can just compute it directly and return. But if we use custom Function, autograd won't trace inside forward.
            # To fix this, we shouldn't use custom Function for the fallback. We should just return the function call.
        else:
...
def cross_selective_scan(
    ...
    def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
        if selective_scan_cuda is None and selective_scan_cuda_core is None and selective_scan_cuda_oflex is None:
            # Fallback to pure PyTorch implementation directly to preserve autograd graph
